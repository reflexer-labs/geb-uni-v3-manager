...
contract GebUniswapV3TwoTrancheManager is ERC20 {
   ...
   */
  function getNextTicks(uint256 _threshold)
    public
    returns (
      int24 _nextLower,
      int24 _nextUpper,
  ...
  function getTokenAmountsFromLiquidity(uint128 _liquidity) public returns (uint256 amount0, uint256 amount1) {
    ...
    (uint256 am0_pos0, uint256 am1_pos0) = getTokenAmountsFromLiquidity(positions[0]);
    (uint256 am0_pos1, uint256 am1_pos1) = getTokenAmountsFromLiquidity(positions[1]);
  ...
  function _deposit(Position storage _position, uint128 newLiquidity) internal returns (uint256 mintAmount) {
    ...
    _mintOnUniswap(_position, nextLowerTick, _nextUpperTick, newLiquidity + compoundLiquidity, abi.encode(msg.sender, collected0, collected1));
  ...
  function _rebalance(Position storage _position) internal {
    ...
    if (_currentLowerTick != _nextLowerTick || _currentUpperTick != _nextUpperTick) {
      // Get the fees
      (uint256 collected0, uint256 collected1) = _burnOnUniswap(_position, _currentLowerTick, _currentUpperTick, _position.uniLiquidity, address(this));
